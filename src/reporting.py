...
from config import CONST_CONFIG, CONST_NAMES
...
def print_constellation_summary(obs, selected, total_per_epoch, const_config):
    """
    ...
    Accepts either CONST_CONFIG (dict-of-dicts) or a plain name-string dict.
    """
    ...
    for prefix in ['G', 'R', 'E', 'C', 'J']:
        ...
        # Resolve display name regardless of dict shape passed in
        if prefix in const_config:
            entry = const_config[prefix]
            sys_name = entry['name'] if isinstance(entry, dict) else entry
        else:
            sys_name = CONST_NAMES.get(prefix, prefix)

        if prefix not in selected:
            print(f"  {sys_name:<8}  — not in file")
            continue
        ...
        snr_mean    = (f"{snr.mean():.1f} dB-Hz"
                       if snr is not None and len(snr) > 0 else "N/A")
    ...
    print()
    print(f"Total multi-GNSS satellites in file : {len(obs.sv.values)}")
